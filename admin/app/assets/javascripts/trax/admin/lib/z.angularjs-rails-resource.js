(function (undefined) {
    ...
    angular.module('rails').factory('railsRootWrappingInterceptor', function () {
        return function (promise) {
            ...
            return promise.then(function (response) {
                if (response.data && response.data.hasOwnProperty(resource.rootName) && !response.data.hasOwnProperty("pager")) {
                  response.data = response.data[resource.rootName];
                } else if (response.data && response.data.hasOwnProperty(resource.rootPluralName) && !response.data.hasOwnProperty("pager")) {
    ...
    angular.module('rails').factory('railsResourceFactory', ['$http', '$q', '$injector', '$interpolate', function ($http, $q, $injector, $interpolate) {
        ...
        function railsResourceFactory(config) {
          // note: removing railsFieldRenamingTransformer to unenforce auto camel casing of results.
            ...
            var transformers = config.requestTransformers || ['railsRootWrappingTransformer', 'railsFieldRenamingTransformer'],
                interceptors = config.responseInterceptors || ['railsRootWrappingInterceptor'];
            ...
            RailsResource.processResponse = function (promise) {
                ...
                return promise.then(function (response) {
                    ...
                    if (angular.isArray(response.data)) {
                        ...
                        if(response.data.hasOwnProperty("pager")) {
                          result = {records: [], pagination: response.data.pager};
                          delete response.data["pager"];
                          var resource_key = Object.keys(response.data)[0];
                          
                          angular.forEach(response.data[resource_key], function (value) {
                            result.records.push(angular.extend(new RailsResource(), value));
                          });
                        } else {
            ...
            RailsResource.search = function (queryParams, context) {
              console.log(queryParams);
                return RailsResource.$post(RailsResource.resourceUrl(context)+"/search", queryParams);
            };
